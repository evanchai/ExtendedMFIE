			...
			var g = {
				...
				getChecked : function() {
					...
					$(".l-checkbox-checked", g.tree).parent().parent("li").each(function() {
						...
						nodes.push({
							target : this,
							data : po.getDataNodeByTreeDataIndex(g.data, treedataindex)
						});
					});
					return nodes;
				},
				...
				//Jeff增加添加本地数据源
				loadLocalData : function(node,data) {
					g.loading.hide();
					g.append(node, data);
					if(p.onSuccess)
						p.onSuccess(data);
				...
				append : function(parentNode, newdata) {
					...
					if($("> .l-children > li:last", treeitem).length > 0)
						po.setTreeItem($("> .l-children > li:last", treeitem)[0], {
							isLast : false
						});
					...
					g.tree.click(function(e) {
						...
						if(!$(obj).hasClass("l-checkbox")) {
							if($(">div:first", treeitem).hasClass("l-selected")) {
								if(p.onBeforeCancelSelect && p.onBeforeCancelSelect({
										data : treenodedata,
										target : treeitem[0]
									}) == false)
									return false;
						...
						//状态：已经张开
							...
							$("> div ." + po.getParentNodeClassName(true), treeitem).removeClass(po.getParentNodeClassName(true)).addClass(po.getParentNodeClassName());
						...
						//状态：没有张开
							...
							$("> div ." + po.getParentNodeClassName(), treeitem).removeClass(po.getParentNodeClassName()).addClass(po.getParentNodeClassName(true));
						}
						p.onClick && p.onClick({
							data : treenodedata,
							target : treeitem[0]
