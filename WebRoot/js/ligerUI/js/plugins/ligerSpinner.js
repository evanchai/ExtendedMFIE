		...
		return this.each(function() {
			...
			var g = {
				...
				addValue : function(num) {
					 ...
					 * Jeff 增加最大值、最小值
					 */
					p.maxValue = p.maxValue || 100;
					p.minValue = p.minValue || 0;
					var value = g.inputText.val();
					value = (parseFloat(value) + num);
					if(value < p.maxValue || value > p.minVal) {
						g.inputText.val(value);
						g.inputText.trigger("change");
					} else if(value == p.maxValue || value == p.maxValue) {
						g.inputText.val(value);
					} else if(value > p.maxValue) {
						g.inputText.val(p.maxValue);
					} else if(value < p.minValue) {
						g.inputText.val(p.minValue);
					}
				},
