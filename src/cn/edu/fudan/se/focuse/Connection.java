...
public class Connection extends Facet{
	...
	{
//		if((preE.getSynonyms().contains(Global.CONNECT)||preE.getSynonyms().contains(Global.CREATE)
//				||preE.getSynonyms().contains(Global.CLOSE))&&isTrue(objE.getObject()))
		if((preE.getSynonyms().contains(Global.CONNECT))&&isTrue(objE.getObject()))
    		postM.put(post, grade);
    	else if(preE.getSynonyms().contains(Global.REGARD)&&objE.getObject().toLowerCase().contains("connect"))
    		postM.put(post, grade);
	}
	@Override
