...
public class Exception extends Facet{
	...
	{
		if((preE.getSynonyms().contains(Global.SHOW)||preE.getSynonyms().contains(Global.GET)
				||preE.getSynonyms().contains(Global.REGARD))&&isTrue(objE.getObject())
				)
    		postM.put(post, grade);
    	else if(isTrue(subE.getSubject()))
    		postM.put(post, grade);
	}
	@Override
